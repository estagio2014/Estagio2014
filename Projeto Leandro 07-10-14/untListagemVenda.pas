...
uses untVenda, untDm, untRelatorios, untRelFiltroVenda, untLancamentos,
  untLogin;

procedure TfrmListagemVenda.btnDetalharClick(Sender: TObject);
begin
  inherited;
  op:= 1;
  frmListagemLancamento.Caption := 'Detalhes da Venda';
...
procedure TfrmListagemVenda.DBGrid1TitleClick(Column: TColumn);
begin
  inherited;
  dm.cdsVenda.IndexFieldNames := Column.FieldName;
end;
...
if(ControlaAcesso = 'Administrador') then
  begin
  if(dm.cdsVenda.FieldByName('situacao').Text <> 'CANCELADO') then
  begin
  if (MessageDlg('Deseja Cancelar esta Venda?', mtConfirmation, [mbYes, mbNo],0)= mrYes) then
  begin
  if (dm.cdsVenda.FieldByName('id_Venda').Text = dm.cdsContasReceb.FieldByName('id_venda').Text) and (dm.cdsContasReceb.FieldByName('situacao').Text = 'Liquidado') then
  begin
    MessageDlg('Erro! VENDA possui parcela(s) liquidada(s). Favor realizar ESTORNO antes de cancelar a VENDA.', mtWarning,[mbOk],0);
    exit;
  end;
