...
procedure TfrmListagemCliente.DBGrid1TitleClick(Column: TColumn);
begin
  inherited;
...
{  if rdbNome.Checked = true then
  ...
  if(cboPesquisar.Text = 'Nome') then
    begin
     dm.cdsCliente.Locate('nomeCli',edtPesquisar.Text,[loPartialKey,loCaseInsensitive]);
     dm.cdsCliente.Locate('Apelido',edtPesquisar.Text,[loPartialKey,loCaseInsensitive]);
    end
  else if(cboPesquisar.Text = 'CPF/CNPJ') then
    begin
     dm.cdsCliente.Locate('cpfCnpj',edtPesquisar.Text,[loPartialKey,loCaseInsensitive]);
    end;
end;
...
begin
  inherited;
  if (cboPesquisar.Text = 'CPF/CNPJ') then
  begin
...
procedure TfrmListagemCliente.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if(Key = VK_F6) then
...
begin
  ...
  Except
    Application.MessageBox('Erro ao Excluir! Este Cliente está sendo utilizado em outro CADASTRO.','Alerta',0);
  end;
end;
...
{   if(cmbRelatorio.Text = 'Filtro Relatório') then
    begin
