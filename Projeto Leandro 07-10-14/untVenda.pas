...
begin
  ...
  begin
    MessageDlg('A quantidade de itens é maior do que há no estoque!', mtError,[mbOk],0);
    edtQtdade.SetFocus;
    exit;
  end;
...
begin
  ...
  begin
    MessageDlg('Campo SITUAÇÃO Obrigatório!', mtError,[mbOk],0);
    cboFormPag.SetFocus;
    exit;
  end;
  ...
  begin
    ...
    begin
      dm.sdsComandoSql.ParamByName('tipo_pag').AsString:='Vista';
    end
    else begin
      dm.sdsComandoSql.ParamByName('tipo_pag').AsString:='Parcelado';
    end;
...
begin
  ...
  begin
   dblCliente.KeyValue := dm.cdsCliente.FieldByName('idCli').AsInteger;
    ...
    MessageDlg('CPF/CNPJ não encontrado ou não existe! Favor fazer o cadastro do cliente.', mtError,[mbOk],0);
    edtCpf.Clear;
    dblCliente.SetFocus;
    exit;
  end;
...
procedure TfrmVenda.edtVendedorChange(Sender: TObject);
begin
  inherited;
edtVendedor.Text := NomeUsuario;
