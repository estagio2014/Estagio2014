object dm: Tdm
  ...
  object sdsContasPagar: TSQLDataSet
    ...
    CommandText = 
      ...
      'arc, vlr_parcela, vlr_receber, data_venc, data_emissao from fatu' +
      'ra inner join lancamento on fatura.id = lancamento.id_Compra whe' +
      're id_compra is not null'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = banco
    Left = 440
  ...
  object sdsContasReceb: TSQLDataSet
    ...
    CommandText = 
      ...
      'c, vlr_parcela, vlr_receber, data_venc, data_emissao from fatura' +
      ' inner join lancamento on fatura.id = lancamento.id_venda where ' +
  ...
  object sdsRelatorioCliente: TSQLDataSet
    ...
    CommandText = 
      'select * from cliente inner join cidade on cliente.id_cid = cida' +
      'de.id_cid '#13#10'where situacao = '#39'A'#39#13#10'order by idCli'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = banco
    Left = 168
  ...
  object sdsRelatorioFornecedor: TSQLDataSet
    ...
    CommandText = 
      'select * from fornecedor inner join cidade on fornecedor.id_cid ' +
      '= cidade.id_cid '#13#10'where situacao = '#39'A'#39#13#10'order by razaosocial'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = banco
    Left = 296
